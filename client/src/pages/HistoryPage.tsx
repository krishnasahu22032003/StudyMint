    ...
    <div className="min-h-screen bg-bg">
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-20">
        ...
        <div className="flex gap-5 items-start">
          <AnimatePresence>
            {sidebarOpen && (
              <motion.aside
                ...
                className="fixed lg:static inset-y-0 left-0 z-40 lg:z-auto"
              >
                <div className="w-72 lg:w-72 h-screen lg:h-auto lg:max-h-[calc(100vh-9rem)] overflow-hidden flex flex-col rounded-none lg:rounded-2xl border-r lg:border border-border bg-surface shadow-soft lg:shadow-elevated">
                  <div className="flex items-center justify-between p-4 border-b border-border flex-shrink-0">
                    ...
                    <button
                      ...
                      className="flex items-center justify-center w-7 h-7 rounded-lg hover:bg-surface-secondary transition-colors duration-200 text-text-tertiary hover:text-text-primary"
                    >
                    ...
                    {!loadingList &&
                      filtered.map((t, i) => (
                        <motion.button
                          ...
                          className={`w-full rounded-xl p-3 text-left transition-all duration-200 group border ${
                            activeNoteId === t._id
                        ...
                        >
                          <div className="flex items-start justify-between gap-2">
                            <p className={`text-xs font-semibold leading-snug truncate transition-colors duration-200 ${
                              activeNoteId === t._id ? "text-accent" : "text-text-primary group-hover:text-accent"
                            }`}>
                              {t.topic}
                            </p>
                            <ChevronRight className={`w-3 h-3 flex-shrink-0 mt-0.5 transition-all duration-200 ${
                              activeNoteId === t._id ? "text-accent" : "text-text-tertiary opacity-0 group-hover:opacity-100"
                            }`} />
                          </div>
          ...
          >
            <div className="relative overflow-hidden rounded-3xl border border-border bg-surface shadow-soft min-h-[calc(100vh-14rem)]">
              <div className="absolute inset-0 bg-gradient-to-br from-accent/4 via-transparent to-gold/4 pointer-events-none" />

              {!sidebarOpen && (
                <button
                  onClick={() => setSidebarOpen(true)}
                  className="absolute top-4 left-4 z-10 flex items-center gap-2 rounded-xl border border-border bg-surface px-3 py-2 text-xs font-medium text-text-secondary hover:text-text-primary hover:border-accent/30 transition-all duration-200 shadow-soft"
                >
